  ...
  // Get transaction graph endpoint
    ...
    try {
        ...
        while (txQueue.length > 0 && nodes.length < depth && iterations < maxIterations) {
            ...
            // Optional: Find transactions that REFERENCE this one (edges pointing FROM this one TO others)
            // This requires an additional query or different data structure if needed frequently
            // const referencingTxs = await db.collection('transactions').find({ references: currentTx.id }).toArray();
            // for (const referencingTx of referencingTxs) {
            //      if (!processedTxIds.has(referencingTx.id)) txQueue.push(referencingTx);
            //      edges.push({ from: currentTx.id, to: referencingTx.id, arrows: 'to', type: 'referenced_by', title: 'Referenced By' });
            // }
